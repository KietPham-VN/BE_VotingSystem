...
public class LectureController(ILectureService service) : ControllerBase
{
    ...
    public async Task<ActionResult<List<Lecture>>> GetLectures(CancellationToken cancellationToken = default)
    {
        var lectures = await service.GetLectures(cancellationToken);
        return Ok(lectures);
    }
    ...
    {
        var lecture = await service.AddLecture(request, cancellationToken);
    ...
    public async Task<ActionResult<Lecture>> UpdateLecture(
        ...
        CancellationToken cancellationToken = default)
    {
        var lecture = await service.UpdateLecture(id, request, cancellationToken);
        return Ok(lecture);
    }
    ...
    [SwaggerOperation(
        ...
        Description = "Deletes a lecture by ID. This will also delete all associated votes. Requires admin privileges.")]
    ...
    public async Task<ActionResult> DeleteLecture(
        ...
        CancellationToken cancellationToken = default)
    {
        await service.DeleteLecture(id, cancellationToken);
        return NoContent();
    }
}
